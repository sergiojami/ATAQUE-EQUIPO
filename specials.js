...
(function(){
  ...
  async function renderSpecials(){
    ...
    c.innerHTML=`<div class="specials-page"><div class="calendar-toolbar"><div><span class="eyebrow">CONTROL DE ACTIVIDADES</span><h3>Especiales</h3><p class="muted">Registra y consulta el número de veces picadas por empleado.</p></div><div class="specials-total">Total registros <strong>${['c','v','cs','ap','b'].reduce((s,k)=>s+total(k),0)}</strong></div></div><div class="specials-card"><div class="specials-table-wrap"><table class="specials-table"><thead><tr><th>Empleado</th><th><span class="special-code c">C</span></th><th><span class="special-code v">V</span></th><th><span class="special-code cs">CS</span></th><th><span class="special-code ap">AP</span></th><th><span class="special-code b">B</span></th><th>Total</th></tr></thead><tbody>${people.map(e=>{const row=map.get(e.id)||{c:0,v:0,cs:0,ap:0,b:0};const sum=['c','v','cs','ap','b'].reduce((s,k)=>s+Number(row[k]||0),0);return `<tr><td><div class="special-employee"><span class="avatar-small">${initials(e.full_name)}</span><div><strong>${esc(e.full_name)}</strong><small>${esc(e.role||'Empleado')}</small></div></div></td>${['c','v','cs','ap','b'].map(k=>`<td><div class="counter"><button aria-label="Restar ${k}" onclick="window.specialChange('${e.id}','${k}',-1)">−</button><strong>${Number(row[k]||0)}</strong><button aria-label="Sumar ${k}" onclick="window.specialChange('${e.id}','${k}',1)">+</button></div></td>`).join('')}<td><span class="special-row-total">${sum}</span></td></tr>`}).join('')}</tbody><tfoot><tr><th>TOTALES</th>${['c','v','cs','ap','b'].map(k=>`<th>${total(k)}</th>`).join('')}<th>${['c','v','cs','ap','b'].reduce((s,k)=>s+total(k),0)}</th></tr></tfoot></table></div><div class="specials-note"><span>Las columnas significan <b>C</b>, <b>V</b>, <b>CS</b>, <b>AP</b> y <b>B</b>.</span>${admin?'<button class="btn secondary" onclick="window.resetSpecials()">↺ Restablecer todos</button>':'<span>Los contadores pueden registrarse desde cualquier usuario.</span>'}</div></div></div>`;
  }

  window.specialChange=async function(employeeId,field,delta){
    if(!current && !admin) return;
    if(!['c','v','cs','ap','b'].includes(field)) return;
    const {data,error}=await db.from('especiales').select('id,c,v,cs,ap,b').eq('empleado_id',employeeId).maybeSingle();
    if(error){alert(error.message);return;}
    const row=data||{c:0,v:0,cs:0,ap:0,b:0};
    const next=Math.max(0,Number(row[field]||0)+delta);
    const payload={empleado_id:employeeId,[field]:next};
  ...
  window.resetSpecials=async function(){
    if(!admin) return;
    if(!confirm('¿Restablecer todos los contadores de Especiales a cero?')) return;
    const r=await db.from('especiales').update({c:0,v:0,cs:0,ap:0,b:0}).neq('empleado_id','00000000-0000-0000-0000-000000000000');
    if(r.error){alert(r.error.message);return;}
    await renderSpecials();
  };
